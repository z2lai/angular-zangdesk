...
export class IssueService {
  ...
  getIssues(): Observable<Issue[]> {
    return this.http.get<Issue[]>(this.issuesUrl).pipe(
      tap(issues => console.log('fetched issues', issues)),
  ...
  updateIssue(updatedIssue: Issue): Observable<any> {
    return this.http
      .put<Issue>(this.issuesUrl, updatedIssue, this.httpOptions)
      .pipe(
  ...
  addIssue(newIssue: Issue): Observable<Issue> {
    ...
    if (Math.random() >= 0.5) {
      return new Observable(function subscribe(subscriber) {
        setTimeout(() => {
          subscriber.error(
            new Error(`Simulated add issue error for id: ${newIssue.id}`),
          );
        }, 1000);
      });
    } else {
      return this.http
        ...
        .pipe(
          tap((newIssue: Issue) =>
            console.log(`Added Issue w/ id=${newIssue.id}`),
          ),
  ...
  deleteIssue(id: number): Observable<Issue> {
    ...
    if (Math.random() >= 0.5) {
      return new Observable(function subscribe(subscriber) {
        setTimeout(() => {
          subscriber.error(
            new Error(`Simulated delete issue error for id: ${id}`),
          );
        }, 1000);
      });
    } else {
  ...
  searchIssuesByName(name: string): Observable<Issue[]> {
    return this.http
      .get<Issue[]>(`${this.issuesUrl}/?name=${name}`)
